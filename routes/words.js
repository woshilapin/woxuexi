...
router.put('/', function(req, res) {
  ...
  if (req.body instanceof Array) {
    ...
    req.body.forEach(function(word) {
      ...
      db.collection('words')
      .update(query, word, function(err, result) {
        if (err === null && result === 1) {
          ...
          if (err === null && result === 0) {
            db.collection('words')
            .insert(word, function(err, result) {
              if (err === null) {
                sendFinal({msg: ''});
              } else {
                sendFinal({msg: err});
    ...
    res.status(403).send({msg: 'The body must be an array.'});
  }
});
...
router.get('/random', function(req, res) {
  ...
  db.collection('words')
  .findOne(query, wordprojection, function(err, result) {
...
router.put('/:word', function(req, res) {
  ...
  db.collection('words')
  .update(query, req.body, function(err, result) {
    if (err === null && result === 1) {
